<?php
/*
 * Файл local/components/mike/books/component.php
 */
...
if(!Loader::includeModule('itbizon.mike')){
    throw new Exception(Loc::getMessage('Модуль не обнаружен'));
}
...
if ($arParams['SEF_MODE'] == 'Y') {
    ...
    if (empty($componentPage)) {
        \Bitrix\Iblock\Component\Tools::process404(
            trim($arParams['MESSAGE_404']) ?: 'Страница на найдена',
            true,
    ...
    if ($componentPage == 'add') {
        if ($arParams['USE_CODE_INSTEAD_ID'] == 'Y') { // если используются символьные коды
            if ( ! (isset($arVariables['ADD_CODE']) && strlen($arVariables['ADD_CODE']) > 0)) {
                $notFound = true;
            }
        } else { // если используются идентификаторы
            if ( ! (isset($arVariables['ADD_ID']) && ctype_digit($arVariables['ADD_ID']))) {
                $notFound = true;
            }
        }
    }
    ...
    if ($componentPage == 'edit') {
        if ($arParams['USE_CODE_INSTEAD_ID'] == 'Y') { // если используются символьные коды
            if ( ! (isset($arVariables['EDIT_CODE']) && strlen($arVariables['EDIT_CODE']) > 0)) {
                $notFound = true;
            }
        } else { // если используются идентификаторы
            if ( ! (isset($arVariables['EDIT_ID']) && ctype_digit($arVariables['EDIT_ID']))) {
                $notFound = true;
            }
        }
    }
     ...
     */
    $componentPage = '';
    if (isset($arVariables['ADD_ID']) && intval($arVariables['ADD_ID']) > 0)
        $componentPage = 'add'; // элемент инфоблока по идентификатору
    elseif (isset($arVariables['ADD_CODE']) && strlen($arVariables['ADD_CODE']) > 0)
        $componentPage = 'add'; // элемент инфоблока по символьному коду
    elseif (isset($arVariables['EDIT_ID']) && intval($arVariables['EDIT_ID']) > 0)
        $componentPage = 'edit'; // раздел инфоблока по идентификатору
    elseif (isset($arVariables['EDIT_CODE']) && strlen($arVariables['EDIT_CODE']) > 0)
        $componentPage = 'edit'; // раздел инфоблока по символьному коду
    else
        $componentPage = 'view'; // главная страница компонента
