$(document).ready(function () {
    ...
    $("#field-action").on("change", function () {
        if($(this).find("option[value="+$(this).val()+"]").prop("disabled") === true) {
            $(this).val('');
        }
...
function getActionId(name) {
    return BX.message("ITB_FINANCE.PERMISSION.EDIT.ACTIONS." + name).toString();
}

function getEntityTypeId(name) {
    return BX.message("ITB_FINANCE.PERMISSION.EDIT.ENTITY." + name).toString();
}

function getEntityId(name) {
    return BX.message("ITB_FINANCE.PERMISSION.EDIT.ENTITY_ID." + name).toString();
}