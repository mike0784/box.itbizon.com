...
/**@var $component CITBRequestTemplateAdd */
            ...
            <form id="form-module">
                <div class="card <?= $isIframe ? "rounded-0 border-bottom-0" : ""?>">
                    <div class="card-header">
                        <?= Loc::getMessage('ITB_FIN.REQ_TEMPLATE.ADD.TEMPLATE.TITLE') ?>
                    </div>
                    <div class="card-body">
                        <?php if($component->getError()) : ?>
                            <div class="alert alert-danger">
                                <strong><?= Loc::getMessage("ITB_FIN.REQ_TEMPLATE.ADD.TEMPLATE.WARNING") ?></strong> <?= $component->getError() ?>
                            </div>
                        <?php endif; ?>

                        <div class="form-group">
                            <label for="field-name"><?= Loc::getMessage('ITB_FIN.REQ_TEMPLATE.ADD.TEMPLATE.FIELD.NAME').$required ?></label>
                            <input id="field-name" class="form-control" type="text" name="DATA[NAME]" value="<?=$data['NAME']?>" required>
                        </div>
                        <div class="form-group">
                            <label for="field-category"><?= Loc::getMessage('ITB_FIN.REQ_TEMPLATE.ADD.TEMPLATE.FIELD.CATEGORY').$required ?></label>
                        ...
                        <div class="form-group">
                            <label for="field-amount"><?= Loc::getMessage('ITB_FIN.REQ_TEMPLATE.ADD.TEMPLATE.FIELD.AMOUNT').$required ?></label>
                            <input id="field-amount" class="form-control" type="number" step="0.01" name="DATA[AMOUNT]" value="<?=$data['AMOUNT']?>" required>
                        </div>
                        <div class="form-group">
                            <label for="field-active"><?= Loc::getMessage('ITB_FIN.REQ_TEMPLATE.ADD.TEMPLATE.FIELD.ACTIVE').$required ?></label>
                        ...
                        <div class="form-group">
                            <label for="field-situation"><?= Loc::getMessage('ITB_FIN.REQ_TEMPLATE.ADD.TEMPLATE.FIELD.SITUATION').$required ?></label>
                            <textarea id="field-situation" class="form-control" name="DATA[COMMENT_SITUATION]" required><?=$data['COMMENT_SITUATION']?></textarea>
                        </div>
                        <div class="form-group">
                            <label for="field-data"><?= Loc::getMessage('ITB_FIN.REQ_TEMPLATE.ADD.TEMPLATE.FIELD.DATA').$required ?></label>
                            <textarea id="field-data" class="form-control" name="DATA[COMMENT_DATA]" required><?=$data['COMMENT_DATA']?></textarea>
                        </div>
                        <div class="form-group">
                            <label for="field-solution"><?= Loc::getMessage('ITB_FIN.REQ_TEMPLATE.ADD.TEMPLATE.FIELD.SOLUTION').$required ?></label>
                            <textarea id="field-solution" class="form-control" name="DATA[COMMENT_SOLUTION]" required><?=$data['COMMENT_SOLUTION']?></textarea>
                        </div>
                        <div class="form-group">
                            <label for="field-company"><?= Loc::getMessage('ITB_FIN.REQ_TEMPLATE.ADD.TEMPLATE.FIELD.COMPANY') ?></label>
                        ...
                        <div class="form-group">
                            <label for="field-vault"><?= Loc::getMessage('ITB_FIN.REQ_TEMPLATE.ADD.TEMPLATE.FIELD.VAULT_ID') ?></label>
                    ...
                    <? $APPLICATION->IncludeComponent('bitrix:ui.button.panel', '', [
                        'BUTTONS' => [
                            [
                                'TYPE' => 'custom',
                                'LAYOUT' => "<button class='ui-btn ui-btn-success'>" . Loc::getMessage("ITB_FIN.REQ_TEMPLATE.ADD.TEMPLATE.BUTTON.ADD") . "</button>"
                            ],
                            'cancel' => '../'
                        ]
                    ]); ?>
