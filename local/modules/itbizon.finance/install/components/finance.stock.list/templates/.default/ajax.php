...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && $cmd == 'import') {
        ...
        if(!CurrentUser::get()->isAdmin()){
            $logicFilter['=AUTHOR_ID'] = CurrentUser::get()->getId();
        }
        ...
        while ($obj = $objList->fetchObject()) {
            ...
            $downloadData[] = [
                ...
                Money::formatfromBase($obj->getAmount()), $obj->getCommentSituation(), $obj->getCommentData(), $fullNameApprove, $obj->getApproverComment(),
                Helper::getEntityList()[$obj->getEntityType()] . ': ' . $obj->getEntityName()
            ];
            $totalAmount += $obj->getAmount();
        }
        $downloadData[] = ['', 'Итого:', '', '', '', '', '', Money::formatFromBase($totalAmount)];
