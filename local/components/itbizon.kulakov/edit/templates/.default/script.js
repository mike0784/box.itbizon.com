$(document).ready(function () {
    ...
    $("#productPopup").on("click", async function () {
        let path = $(this).data('path');
        let e =  $("#submit");
        let form = $(e.parent("form"));
        let newID = null;

        if(path.indexOf('invoiceID=0') !== false) {
            newID = await saveform(form.attr('action'), form.serialize());
            path = path.replace('invoiceID=0', 'invoiceID='+newID);
        }
        ...
        $.ajax({
            ...
            success: function (response) {
                createPopup(response.data, "#editProduct", function() {
                    console.log(newID);
                    if(newID)
